"""Constant mapping dictionaries for code generation and linting.

These maps are immutable lookup tables used by both the importer (for code generation)
and the linter (for detecting and fixing common issues).
"""
...
CONDITION_OPERATOR_REVERSE_MAP: dict[str, str] = {
    v: k for k, v in CONDITION_OPERATOR_MAP.items()
}
...
PSEUDO_PARAMETER_REVERSE_MAP: dict[str, str] = {
    v: k for k, v in PSEUDO_PARAMETER_MAP.items()
}
...
PARAMETER_TYPE_MAP: dict[str, str] = {
    ...
    "List<AWS::EC2::AvailabilityZone::Name>": (
        "ParameterType.LIST_AWS_EC2_AVAILABILITY_ZONE_NAME"
    ),
...
PARAMETER_TYPE_REVERSE_MAP: dict[str, str] = {
    v: k for k, v in PARAMETER_TYPE_MAP.items()
}
